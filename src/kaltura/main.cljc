(ns kaltura.main
  "Kotodama WASM entrypoint for the Kaltura clean-room actor (L4) — Clojure port.
  ...
  facts (`kaltura.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "kaltura")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"kaltura.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "kaltura-compat" :tier tier :entities entities} 200])
...
(defn start! [] :kaltura-compat/ready)
